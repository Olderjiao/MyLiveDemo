 ...
 * @date: 2019 2019/12/31 9:44
 */
public class RankItemAdapter extends BaseQuickAdapter<StartRankEntity.MessageBean.RankStarBean, RankItemAdapter.RankItemHolder> {
    private static ArrayList<Integer> levelIcons = new ArrayList<>();
    private List<StartRankEntity.MessageBean.RankStarBean> beans;
     ...
     */
    public RankItemAdapter(int layoutResId, @Nullable List<StartRankEntity.MessageBean.RankStarBean> data) {
     ...
     */
    @Override
    protected void convert(RankItemHolder helper, StartRankEntity.MessageBean.RankStarBean item) {
    }
    ...
    public void onBindViewHolder(RankItemHolder holder, int position) {
        ...
        for(int i = 1;i <= levelIcons.size();i++){
            if(beans.get(position).getLevel()-1 == i){
                Glide.with(mContext)
        ...
        //判断是否在直播
        if(beans.get(position).getIsInLive() == 1){
    ...
    class RankItemHolder extends BaseViewHolder{
